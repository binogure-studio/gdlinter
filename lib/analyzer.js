...
const analyzeFunction = function (filename, data, scope, originalFunctionName, nesting) {
  ...
  call.forEach((key) => {
    if (/[^\.]+\.connect\(/.test(key) && !/(async_ready|_init|_ready|connect_signals)/.test(originalFunctionName)) {
      console.error(filename, originalFunctionName, `Connecting signals from running calls ${key}`)
    }
  })
